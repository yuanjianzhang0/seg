...
# 加载训练好的模型权重
checkpoint = torch.load('model_checkpoints/run3/best_model.pth')
model.load_state_dict(checkpoint)
model.eval()

# 创建保存预测结果的目录
os.makedirs('XCADLabeled/test/predictions', exist_ok=True)
...
with torch.no_grad():
    for images, masks, image_names in test_loader:
        ...
        for output, mask, image_name in zip(outputs, masks, image_names):
            ...
            output_image.save(f'XCADLabeled/test/predictions/{image_name}')
            ...
            # 保存可视化结果
            visualize(images[0].cpu(), masks[0], output[0], f'XCADLabeled/test/predictions/visualization_{image_name}.png')

# 保存所有图片的预测指标
metrics_df = pd.DataFrame(metrics, columns=['Image', 'Precision', 'Recall', 'IoU', 'F1'])
metrics_df.to_csv('XCADLabeled/test/predictions/metrics.csv', index=False)

# 计算总体指标
mean_metrics = metrics_df[['Precision', 'Recall', 'IoU', 'F1']].mean(axis=0)
with open('XCADLabeled/test/predictions/overall_metrics.txt', 'w') as f:
...
print("推理完成，结果已保存到 'XCADLabeled/test/predictions' 目录中。")
